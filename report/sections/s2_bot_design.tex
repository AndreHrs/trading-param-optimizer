...
\begin{equation}
  ...
  point. The degenerate optimum was observed empirically --- MRFO converged
  ...
  $x_0 \in [5,50]$ and the long window $x_1 \in [51,200]$ are optimised. The
  non-overlapping constraint (short $<$ long) is guaranteed by construction
  ...
  structure is identical to Strategy 1 ($x_0 \in [5,50]$, $x_1 \in
  ...
  optimised jointly. Sharing one $\alpha$ value across both EMAs as in
  \eqref{eqn:ewma} constrains both series to decay at identical rates; the
  ...
  \textit{Strategy 4 --- EMA with independent $\alpha$ ($d=7$).} Short span,
  ...
  contrast that shared $\alpha$ eliminates. The expanded search space ($d=7$
  ...
  dimensionality is a genuine concern when each optimiser is limited to 30 runs.
